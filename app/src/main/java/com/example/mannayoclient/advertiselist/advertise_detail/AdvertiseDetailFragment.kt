...
import com.example.mannayoclient.writelist.Item
...
class AdvertiseDetailFragment : Fragment(R.layout.advertisedetail_frag) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.imageView88.setOnClickListener {
            retrofitService.service.setLike(memberid, boardid)
        ...
        retrofitService.service.getBoard(boardid).enqueue(object : Callback<BoardResponseDto> {
            override fun onResponse(
                ...
                if (receive.isVote) {
                    ...
                    retrofitService.service.getVote(receive.boardId, memberid)
                        .enqueue(object : Callback<List<VoteResponseDto>> {
                            override fun onResponse(
                ...
                if (receive.isProfile) {
                    retrofitService.service.getMyProfileImage(memberid)
        ...
        if (comment!! > 0) {
            retrofitService.service.getComment(boardid)
                .enqueue(object : Callback<List<commentDto>> {
                    override fun onResponse(
                        ...
                        for (c: commentDto in receive) {
                            if (c.depth == 1) {
                                list.add(TodayReplyModel(TodayReplyModel.reply1,
                                    c.nickname,
                                    c.date,
                                    c.contents,
                                    c.id,
                                ...
                                list.add(TodayReplyModel(TodayReplyModel.reply2,
                                    c.nickname,
                                    c.date,
                                    c.contents,
                                    c.id,
        ...
        binding.SendReply.setOnClickListener {
            ...
            if (depth == 1L) {
                ...
                retrofitService.service.setReply2(memberid,
                    commentid,
                    binding.replyEdit.text.toString()).enqueue(object : Callback<ReceiveOK> {
                    override fun onResponse(call: Call<ReceiveOK>, response: Response<ReceiveOK>) {
                        if (response.isSuccessful) {
